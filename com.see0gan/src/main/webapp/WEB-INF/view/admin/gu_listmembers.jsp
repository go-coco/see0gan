...
<c:set var="guestList" value="${membersMap.guestList }" />
	...
	<body id="page-top">
		<div id="wrapper">
			...
			<div id="content-wrapper" class="d-flex flex-column">
				...
				<select id="keyfilter" name="keyfilter" onChange="selectfilter()">
					<option value="key1">회원ID</option>
					<option value="key2">회원이름</option>

				</select>

					<input type="text" id="keyword" name="keyword"  placeholder="회원ID 또는 이름 입력"><br>
				...
				<br>
	 	<c:if test="${allguestMember !=null}">
		<c:choose>
			<c:when test="${allguestMember >200 }">
			
		<c:forEach var="page" begin="1" end="20" step="1">
			<c:if test="${section >1 && page==1 }"> <!--페이지번호가 1일때 pre나오게 하기 -->
				<a href="${contextPath }/admin/gu_listmembers.sgg?section=${section-1}&pageNum=${(section-1)*20+1}">%nbsp; pre </a>
			</c:if>
			...
			<c:if test="${page==10} }"><!--페이지수가 10일때 다음으로 next나오게 하기  -->
				<a href="${contextPath }/admin/gu_listmembers.sgg?section=${section+1}&pageNum=${section*20+1}">&nbsp; next</a>
			</c:if>	
		...
		<c:when test="${allguestMember <200 }">
			<c:forEach var="page" begin="1" end="${allguestMember/20+1 }" step="1">
				<c:choose>
					<c:when test="${page==pageNum }">
						<a class="sel-page" href="${contextPath }/admin/gu_listmembers.sgg"
						>${page }</a>
					</c:when>
					<c:otherwise>
						<a class="no-uline" href="${contextPath }/admin/gu_listMembers.sgg
						section=${section}
						&pageNum=${page}
						">${page }</a>
					</c:otherwise>
